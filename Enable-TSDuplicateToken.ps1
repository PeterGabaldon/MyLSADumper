...
"@
  ...
  Add-Type -MemberDefinition $signature -Name peterPriv -Namespace peterPriv
  $peterPriv = [peterPriv.peterPriv]
  [long]$luid = 0

  $tokPriv1Luid = New-Object peterPriv.peterPriv+TokPriv1Luid
  $tokPriv1Luid.Count = 1
  $tokPriv1Luid.Luid = $luid
  $tokPriv1Luid.Attr = [peterPriv.peterPriv]::SE_PRIVILEGE_ENABLED

  $retVal = $peterPriv::LookupPrivilegeValue($null, "SeDebugPrivilege", [ref]$tokPriv1Luid.Luid)

  [IntPtr]$htoken = [IntPtr]::Zero
  $retVal = $peterPriv::OpenProcessToken($peterPriv::GetCurrentProcess(), [peterPriv.peterPriv]::TOKEN_ALL_ACCESS, [ref]$htoken)
  
  
  $tokenPrivileges = New-Object peterPriv.peterPriv+TOKEN_PRIVILEGES
  $retVal = $peterPriv::AdjustTokenPrivileges($htoken, $false, [ref]$tokPriv1Luid, 12, [IntPtr]::Zero, [IntPtr]::Zero)
  ...
  $retVal = $peterPriv::OpenProcessToken($process.Handle, ([peterPriv.peterPriv]::TOKEN_IMPERSONATE -BOR [peterPriv.peterPriv]::TOKEN_DUPLICATE), [ref]$hlsasstoken)

  [IntPtr]$dulicateTokenHandle = [IntPtr]::Zero
  $retVal = $peterPriv::DuplicateToken($hlsasstoken, 2, [ref]$dulicateTokenHandle)

  $retval = $peterPriv::SetThreadToken([IntPtr]::Zero, $dulicateTokenHandle)
  if(-not($retVal)) {
    [System.Runtime.InteropServices.marshal]::GetLastWin32Error()
  }
}
